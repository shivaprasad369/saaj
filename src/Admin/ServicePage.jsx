...
export default function ServicePage() {
  ...
  const handleSubmit = async () => {
    try {
      setLoading(true);
      const res = await axios.post(`https://server-442v.onrender.com/api/services/`, {
       name:name,
        photo: photo,
        nameId: nameId,
      });
  ...
  return (
    <React.Fragment>
      ...
      >
        <DialogTitle>Service page images</DialogTitle>
        <DialogContent>
          ...
          <form className="relative flex flex-col gap-5 border-[1px]  w-full  border-blue-600 p-5 rounded-md ">
              ...
              <select name="" id="" value={nameId} onChange={(e)=>setNameId(e.target.value)} className="border-[1px] p-2 border-gray-200 h-[3rem] bg-gray-100">
                <option value={''} >Select Name</option>
                  ...
                  <option value={1} >
                  Wedding Planner (4:16 accept ration & only png for Best view)
                  </option>
                  <option value={2} >
                  Vendors And Artist Management (4:16 accept ration & only png for Best view) 
                  </option>
                  <option value={3} >
                  Decor And Design (4:16 accept ration & only png for Best view)
                  </option>
                  <option value={4} >
                  Corporate Events (4:16 accept ration & only png for Best view)
                  </option>
              </select>
              {/* <input
                type="text"
