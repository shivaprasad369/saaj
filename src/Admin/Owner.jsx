...
export default function Owner() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setLoading(true);
      const res = await axios.post(`https://server-442v.onrender.com/api/owner/`, {
        photo:photo,
        name:name,
        nameId:nameId
      });
  ...
  return (
    <React.Fragment>
      ...
      >
        <DialogTitle>Members images</DialogTitle>
        <DialogContent>
          ...
          <form className="relative flex flex-col gap-5 border-[1px]  w-full  border-blue-600 p-5 rounded-md ">
              ...
              <select name="" id="" value={nameId} onChange={(e)=>setNameId(e.target.value)} className="border-[1px] p-2 border-gray-200 h-[3rem] bg-gray-100">
                <option value={''} >Select Name</option>
                  ...
                  <option value={1} >
                  Sanjay Gupta [Founder] (4:4 accept ration & only png for Best view)
                  </option>
                  <option value={2} >
                  Mohit Singh [HOSPITALITY & LOGISTICS MANAGER] (4:4 accept ration & only png for Best view) 
                  </option>
                  <option value={3} >
                  Varun Sharma [PRODUCTION MANAGER] (4:4 accept ration & only png for Best view)
                  </option>
